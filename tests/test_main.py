...
def mock_settings():
    with patch("scout.main.Settings") as MockSettings:
        ...
        settings.PUMPFUN_ENABLED = False
        settings.ONCHAIN_SIGNALS_ENABLED = False
        settings.QUALITY_GATE_ENABLED = False
        settings.SOCIAL_ENRICHMENT_ENABLED = False
        settings.POOL_WATCHER_ENABLED = False
        settings.MAX_CANDIDATES_PER_CYCLE = 50
        settings.GOPLUS_FAIL_CLOSED = True
        settings.REENTRY_MIN_CONVICTION = 40.0
        settings.REENTRY_DIP_PCT = 20.0
        settings.INJECTIONS_DB_PATH = ":memory:"
...
def mock_db():
    ...
    db.commit = AsyncMock()
    db.prune_old_data = AsyncMock(return_value=0)
    return db
...
async def test_run_cycle_dry_run(mock_db, mock_session, mock_settings):
    ...
    with patch("scout.main.fetch_trending", new_callable=AsyncMock, return_value=[token]), \
         ...
         patch("scout.main.fetch_smart_money_injections", new_callable=AsyncMock, return_value=[]), \
